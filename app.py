...
def build_pdf(patient, blocks):
    ...
    pdf.cell(
        0,
        8,
        pdf_safe(f"Diabetes: {'Yes' if patient['diabetes'] else 'No'}"),
        ln=True,
    )
    pdf.ln(4)
    ...
    pdf.cell(
        0,
        6,
        pdf_safe(f"Total Score (0-100): {blocks['total_score']:.1f}"),
        ln=True,
    )
    pdf.cell(
        0,
    ...
    pdf.cell(
        ...
        6,
        pdf_safe(
            f"Inflammation: {blocks['inflam_score']:.1f}/25 "
            f"({blocks['inflam_label']})"
        ),
        ln=True,
    )
    pdf.cell(
        ...
        6,
        pdf_safe(
            f"Oxidative / Hb-MCV: {blocks['oxid_score']:.1f}/25 "
            f"({blocks['oxid_label']})"
        ),
        ln=True,
    )
    pdf.cell(
        ...
        6,
        pdf_safe(
            f"Endothelial: {blocks['endo_score']:.1f}/25 "
            f"({blocks['endo_label']})"
        ),
        ln=True,
    )
    pdf.cell(
        ...
        6,
        pdf_safe(
            f"Metabolic / IR / Liver: {blocks['metab_score']:.1f}/25 "
            f"({blocks['metab_label']})"
        ),
        ln=True,
    )
    pdf.ln(4)
    ...
    pdf.multi_cell(
        ...
        pdf_safe(
            ...
            "guidance only and does not replace clinical judgment or diagnostic "
            "workup. Please correlate with the full clinical context."
        ),
    )
...
def main():
    ...
    if st.button("Calculate & Generate Report"):
        ...
        with d1:
            st.write(
                f"Inflammation score: **{infl['inflam_score']:.1f} / 25 "
                f"({inflam_label})**"
            )
            st.write(
                f"Oxidative / Hb–MCV score: **{oxid['oxid_score']:.1f} / 25 "
                f"({oxid_label})**"
            )
        with d2:
            st.write(
                f"Endothelial score: **{endo['endo_score']:.1f} / 25 "
                f"({endo_label})**"
            )
            st.write(
                f"Metabolic / IR / Liver score: **{metab['metab_score']:.1f} / 25 "
                f"({metab_label})**"
            )

        st.subheader("5. Overall Score")
        st.write(
            f"Total CBC + Biochem Metabolic Health Score: "
